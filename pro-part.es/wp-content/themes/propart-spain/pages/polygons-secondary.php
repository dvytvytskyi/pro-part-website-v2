    ...
    },
    {
        ...
        async function fetchSecondaryProjects() {
            ...
            if (queryString) {
                ...
                const projectIdsBase64 = sessionStorage.getItem("projectIdsBase64Secondary");
                const projectIdsArray = projectIdsBase64
                ...
                if (queryParams.has("propertyType")) {
                        const propertyType = decodeURIComponent(queryParams.get("propertyType")).split(",");
                        body.propertyType = propertyType;
                    }
                if (queryParams.has("maxPrice"))
                    body.maxPrice = queryParams.get("maxPrice");
				if (queryParams.has("shareId"))
                    body.shareId = queryParams.get("shareId");
                 if (queryParams.has("maxPrice"))
                    body.maxPrice = queryParams.get("maxPrice");
                if (queryParams.has("minSize"))
                    body.minSize = queryParams.get("minSize");
                if (queryParams.has("minSize"))
                    body.minSize = queryParams.get("minSize");
                if (queryParams.has("maxSize"))
                    body.maxSize = queryParams.get("maxSize");
                    if (queryParams.has("propertyType")) {
                        const propertyType = decodeURIComponent(queryParams.get("propertyType")).split(",");
                        body.propertyType = propertyType;
                    }
                if (queryParams.has("bedrooms")) {
                    body.rooms = queryParams.get("bedrooms").split(",");
                }
                if (queryParams.has("subAreas")) {
                    body.provinces = queryParams.get("subAreas").split(",");
                }
                if (queryParams.has("projectName"))
                    body.name = queryParams.get("projectName");
				if (queryParams.has("mustHave")) {
						const propertyType = decodeURIComponent(queryParams.get("mustHave")).split(",");

						// Ініціалізація об'єкта для результату
						const result = {};

						// Проходимося по кожній категорії з filterModalData
						Object.keys(filterModalData).forEach((category) => {
							// Фільтруємо значення propertyType, які відповідають поточній категорії
							const matchingValues = propertyType.filter((item) =>
								filterModalData[category].includes(item.trim())
							);

							// Якщо знайдено відповідні значення, додаємо їх у результат
							if (matchingValues.length > 0) {
								result[category] = matchingValues;
							}
						});

						console.log(result);
						body.amenitiesFilter = result;
					}
                ...
                try {
                    const response = await fetch(
                        `https://crm.server.pro-part.es/api/v1/secondary-projects/integration/projects/list?accessKey=${accessKey}&secretKey=${secretKey}&page=${parseInt(pageParam)}&size=30`,
                        {
                            method: "POST",
                            headers: {
                    ...
                    totalSecondary.textContent = `projects: ${data.totalObjects}`;
                    ...
                    const projectsContainer = document.getElementById("projectsSecondaryContainer");
        ...
        function renderFiltersFromQuerySecondary() {
            ...
            if (queryParams.has("propertyType")) {
                const propertyType = decodeURIComponent(queryParams.get("propertyType")).split(",");
                propertyType.forEach((subArea) => {
                    addFilterItem(propertyType, "propertyType", propertyType);
                });
            }
...
async function shareProjects() {
        const projectIdsBase64 = sessionStorage.getItem("projectIdsBase64Secondary");
        const secondaryProjects = projectIdsBase64
            ? projectIdsBase64.split(",").map(id => id.trim())
            : [];
        ...
        try {
            ...
            try {
                ...
                const generatedLink = responseData._id
                    ? `https://pro-part.es/polygons-secondary/?shareId=${responseData._id}&&visible=Secondary&page=0`
                    : "";
